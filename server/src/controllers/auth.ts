...
import { userRepository, organizationRepository, studyFieldRepository } from "../DAL";
...
export const login = async ({ email, password }: models.dtos.UserDto) => {
    ...
    const userOrganizatios = organizations.filter(org => org.users.includes(userFromDb.id || ""));
